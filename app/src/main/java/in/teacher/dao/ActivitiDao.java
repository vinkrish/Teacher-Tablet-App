...
public class ActivitiDao {
    ...
    public static List<Long> getActivityIds(long examId, int subjectId, int sectionId, SQLiteDatabase sqliteDatabase) {
        ...
        c.moveToFirst();
        while (!c.isAfterLast()) {
            idList.add(c.getLong(c.getColumnIndex("ActivityId")));
            c.moveToNext();
        }
        c.close();
        return idList;
